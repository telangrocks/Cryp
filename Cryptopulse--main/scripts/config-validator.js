...
function validateBackendConfig() {
  ...
  if (env.CASHFREE_CLIENT_ID && env.CASHFREE_CLIENT_ID !== '') {
    if (hasWeakPattern(env.CASHFREE_CLIENT_ID)) {
      results.errors.push('CASHFREE_CLIENT_ID appears to be a placeholder');
      results.failed++;
      logError('CASHFREE_CLIENT_ID validation failed');
    } else {
      results.passed++;
      logSuccess('CASHFREE_CLIENT_ID is valid');
    }
  }
