...
async function runAllTests(environment = 'development', options = {}) {
  ...
  // In CI or when explicitly requested, skip performance tests to keep pipelines stable and free
  const skipPerformance = process.env.CI === 'true' || process.env.SKIP_PERFORMANCE_TESTS === 'true';
  const suites = Object.keys(TESTING_CONFIG.testSuites).filter(suite => {
    if (suite === 'performance' && skipPerformance) {
      logWarning('Skipping performance tests due to CI environment or SKIP_PERFORMANCE_TESTS=true');
      return false;
    }
    return true;
  });
