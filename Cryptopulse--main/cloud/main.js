...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    if (res.statusCode >= 400) {
      utils.logging.logError('HTTP Request Error', logData);
    } else {
      utils.logging.logInfo('HTTP Request', logData);
    }
  });
...
// Service routes - ensure proper router mounting
...
app.use((err, req, res, next) => {
  // Log the error
  utils.logging.logError('Unhandled Error', {
    error: err.message,
    stack: err.stack,
    url: req.url,
    method: req.method,
