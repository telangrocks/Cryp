...
export const secureStorage = {
  async setItem(key: string, value: string): Promise<void> {
    try {
      ...
      const encryptedHex = Array.from(result).map(b => b.toString(16).padStart(2, '0')).join('');
      localStorage.setItem(key, encryptedHex);
    } catch (error) {
  ...
  async getItem(key: string): Promise<string | null> {
    try {
      ...
      const dataArray = new Uint8Array(encrypted.match(/.{2}/g)!.map(byte => parseInt(byte, 16)));
